...
export const fetchLoadsByLocation = async (location: string ) =>{
  if(!location) return undefined;
  const provinciaID = getProvinciaIdByName(location) ?? '';

  let {data:ubicacionInicial, error: errorUbicacion} = await supabase.from('ubicaciones').select('*').eq('provincia_id', provinciaID)
  console.log(ubicacionInicial);
  // let { data: cargas, error:ErrorTotalLoads } = await supabase
  // .from('cargas')
  // .select('*')
  // .eq('ubicacioninicial_id', provinciaID)
  // .returns<LoadSupabaseUnit[]>(); 
  // if(ErrorTotalLoads) throw ErrorTotalLoads;
  // let fullLoads: Load[] = [];
  // try{
...
const loadsMapper = async (carga: LoadSupabaseUnit): Promise<Load | undefined> => {
    try {
      ...
      const {data: tipoEquipo, error: errorTipoEquipo} = await supabase.from('tipoequipo').select('descripcion').eq('id', carga.tipo_equipo).maybeSingle();
